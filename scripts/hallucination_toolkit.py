...
class OpenAIBackend:
    ...
    def extract_content_from_choice(self, choice):
        """Extract content from OpenAI choice, handling both regular and reasoning models."""
        if not choice or not hasattr(choice, 'message'):
            return ""
        
        message = choice.message
        
        # For reasoning models (gpt-5 series), check for reasoning field first
        if hasattr(message, 'reasoning') and message.reasoning:
            # Reasoning models may have both reasoning and content
            reasoning = message.reasoning or ""
            content = message.content or ""
            # Return both if available, otherwise just content
    ...
    def responses_create(self, input_text: str, **kwargs):
        """Create response using OpenAI responses endpoint for GPT-5 models."""
        params = {
            "model": self.model,
            "input": input_text,
            "reasoning": {"effort": getattr(self, "reasoning_effort", kwargs.get("reasoning_effort", "medium"))}
        }
        
        # Note: verbosity parameter may not be supported in current API version
        # Note: responses endpoint doesn't support max_tokens parameter
        # It has its own length control through the reasoning effort level
            
        return self.client.responses.create(**params)

    def is_gpt5_model(self):
        """Check if current model is a GPT-5 reasoning model."""
        return self.model in ["gpt-5", "gpt-5-mini", "gpt-5-nano"]

    def chat_create(self, messages: List[Dict], **kwargs):
        # For GPT-5 models, use responses endpoint
        if self.is_gpt5_model():
            # Convert messages to single input text (take the last user message)
            input_text = ""
            ...
            resp = self.responses_create(input_text, **kwargs)
            
            # Convert response to chat completion format for compatibility
            class MockChoice:
                def __init__(self, output_text):
                    self.message = MockMessage(output_text)
            ...
            return MockResponse(resp.output_text)
        ...
        if "max_tokens" in params and self.model in ["gpt-4.1", "gpt-4.1-mini", "gpt-4.1-nano"]:
            params["max_completion_tokens"] = params.pop("max_tokens")
        
        return self.client.chat.completions.create(**params)

    def multi_choice(self, messages: List[Dict], n: int = 1, **kwargs):
        # For GPT-5 models, always create multiple individual requests
        if self.is_gpt5_model():
            choices = []
            for _ in range(n):
        ...
        # For regular models, try batch request first
        try:
...
def _choices_to_decisions(backend: OpenAIBackend, choices) -> List[str]:
    outs = []
    for ch in choices:
        content = ""
        try: 
            content = backend.extract_content_from_choice(ch)
        except Exception: 
...
def estimate_event_signals_sampling(
    ...
    post_decisions = _choices_to_decisions(backend, choices)
    ...
    for sk in skeletons:
        ...
        dec_k = _choices_to_decisions(backend, choices_k)
        if sleep_between > 0: time.sleep(sleep_between)
...
def generate_answer_if_allowed(
    ...
    try:
        return backend.extract_content_from_choice(resp.choices[0])
    except Exception:
        return ""
